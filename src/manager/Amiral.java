...
//  changer le next par une parcours liste simple (2 case par 2 case )
  ...
  private LinkedList<Case> Parcours(Robot robot, Case arrivee){
    ...
    for(int i=0; i<2; i++){
      ...
      //if(lr<l && robot.verif_depl(Direction.SUD, data.getCarte().getCase(lr+1,cr))){ WHY VERI_depl comme ca ??? ROBIN ///
      if(lr<l && robot.verif_depl(Direction.SUD, data.getCarte() )){
        lr++;
        parcours.add(data.getCarte().getCase(lr+1,cr));
        robot.deplacer(Direction.SUD, data.getCarte());
      }
      else if(lr>l && robot.verif_depl(Direction.NORD, data.getCarte())){
        lr--;
        parcours.add(data.getCarte().getCase(lr-1,cr));
        robot.deplacer(Direction.NORD, data.getCarte());
      }
      else if(lr==l && (b_est || b_ouest)){
        if(robot.verif_depl(Direction.SUD, data.getCarte())){
          lr++;
          parcours.add(data.getCarte().getCase(lr+1,cr));
          robot.deplacer(Direction.SUD, data.getCarte());
        }
        else if(robot.verif_depl(Direction.NORD, data.getCarte())){
          lr--;
          parcours.add(data.getCarte().getCase(lr-1,cr));
          robot.deplacer(Direction.NORD, data.getCarte());
        }
      ...
      if(cr<c && robot.verif_depl(Direction.EST, data.getCarte())){
        cr++;
        parcours.add(data.getCarte().getCase(lr,cr+1));
        robot.deplacer(Direction.EST, data.getCarte());
      }
      else if(cr>c && robot.verif_depl(Direction.OUEST, data.getCarte())){
        cr--;
        parcours.add(data.getCarte().getCase(lr,cr-1));
        robot.deplacer(Direction.OUEST, data.getCarte());
      }
      else if(cr==c && (b_nord || b_sud)){
        if(robot.verif_depl(Direction.EST, data.getCarte())){
          cr++;
          parcours.add(data.getCarte().getCase(lr,cr+1));
          robot.deplacer(Direction.EST, data.getCarte());
        }
        else if(robot.verif_depl(Direction.OUEST, data.getCarte())){
          cr--;
          parcours.add(data.getCarte().getCase(lr,cr-1));
          robot.deplacer(Direction.OUEST, data.getCarte());
        }
